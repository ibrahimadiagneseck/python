...
def determinantMatriceTriangulaire(matrice):
    ...
    det = 1
    for i in range(taille): # expl: taille = 3
        det *= matrice[i][i]
    return det
#-------------------------------------
...
# remplaceLigne(matrice, 2, 1, 2) # remplaceLigne(matrice, indicePivot, indiceLigne, colonnePivot)
for i in range(n1 - 1, 0, -1): # boucle i (lignes pivots) de n à 1
    for j in range(i - 1, -1, -1): # boucle j (lignes) de (lignes pivots)-1 à 0
...
if(det == 0):
    ...
    print('La solution : ', triangleInferieur(matrice))
